package com.customview.ticketingapps.customView
...
import com.customview.ticketingapps.R
...
class SeatsView : View {
    ...
    private val titlePain = Paint(Paint.FAKE_BOLD_TEXT_FLAG)
...
//        Mengatur warna ketika seat sudah di booking
        if (seat.isBooked) {
            backgroudPaint.color = ResourcesCompat.getColor(resources, R.color.grey_200, null)
            ...
            backgroudPaint.color = ResourcesCompat.getColor(resources, R.color.blue_500, null)
...
//        background
        canvas?.translate(
            seat.x as Float, seat.y as Float
        )
...
//        Menembalikan ke pengeturan sebelumnya
        canvas?.restore()
    }
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        val widthCollumnOne = (halfOfWidth - 300F)..(halfOfWidth - 100F)
        val widthCollumnTwo = (halfOfWidth + 100F)..(halfOfWidth + 300F)
        ...
        if (event?.action == ACTION_DOWN) {
            when {
                event.x in widthCollumnOne && event.y in heightRowOne -> booking(0)
                event.x in widthCollumnTwo && event.y in heightRowOne -> booking(1)
                event.x in widthCollumnOne && event.y in heightRowTwo -> booking(2)
                event.x in widthCollumnTwo && event.y in heightRowTwo -> booking(3)
                event.x in widthCollumnOne && event.y in heightRowTree -> booking(4)
                event.x in widthCollumnTwo && event.y in heightRowTree -> booking(5)
                event.x in widthCollumnOne && event.y in heightRowFour -> booking(6)
                event.x in widthCollumnTwo && event.y in heightRowFour -> booking(7)
            }
        }
        return true
    }
